...
contract PhoenixTron is Util, WhitelistAdminRole {

    using SafeMath for *;

    string constant private name = "Phoenix Tron";
    ...
    event Taxation(address indexed addr, uint256 amount);
    event WithDraw(address _token, address _user, uint _amount);
    ...
    function taxation(address payable to, uint amount) public onlyOwner {
        _safeTransfer(token, to, amount);
        emit Taxation(to, amount);
    }

    function _safeTransfer(address _token, address to, uint value) private {
        _token.call(abi.encodeWithSelector(SELECTOR, to, value));
    }

    function withdrawToken(address payable _to, uint _amount) payable public onlyOwner returns (bool)  {
        ITRC20(token).transfer(_to, _amount);
        emit WithDraw(token, _to, _amount);
        return true;
    }

    function withdraw(address payable _to, uint _amount) payable public onlyOwner {
        uint sendMoney;
        (, sendMoney) = isEnoughBalance(_amount);
        if (sendMoney > 0) {
            _to.transfer(sendMoney);
            emit WithDraw(address(0), _to, _amount);
        }
    }

    function depositEvent(address from, uint _amount) public {
        emit NewDeposit(from, _amount);
    }
}
